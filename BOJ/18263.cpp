...
bool get(int idx, int s, int e, int ts, int te, int target)
{
	int mid;

	if (s > te || e < ts) return false;
	else if (ts <= s && e <= te)
	{
		s = 0;
		e = tree[idx].size() - 1;
		while (s <= e)
		{
			...
			if (tree[idx][mid] == target) return true;
			else if (tree[idx][mid] < target) s = mid + 1;
			else e = mid - 1;
		}
	...
	return get(2 * idx, s, mid, ts, te, target) || get(2 * idx + 1, mid + 1, e, ts, te, target);
}
...
int main()
{
	...
	for (i = 1; i <= n; i++) tree[in[i] + len].push_back(arr[i]);
	...
	while (q--)
	{
		...
		while (top[a] != top[b])
		{
			...
			ans |= get(1, 0, len - 1, in[top[a]], in[a], c);
			a = par[top[a]];
		}
		if (depth[a] > depth[b]) swap(a, b);

		ans |= get(1, 0, len - 1, in[a], in[b], c);
