#include<iostream>
#include<vector>
#include<stack>
#include<map>
using namespace std;

const int SIZE = 1 << 18;
const int len = 1 << 17;
int n;
int par[100010];
int depth[100010];
stack<pair<pair<int, int>, int>> st;
vector<pair<pair<int, int>, pair<int, int>>> v;
vector<pair<int, int>> query;
vector<int> tree[SIZE];
map<int, int> m[100010];

void update(int idx, int s, int e, int ts, int te, int v)
{
	int mid;

	if (s > te || e < ts) return;
	
	tree[idx].push_back(v);

	if (ts <= s && e <= te) return;

	mid = (s + e) / 2;

	update(2 * idx, s, mid, ts, te, v);
	update(2 * idx + 1, mid + 1, e, ts, te, v);
}

int find(int x)
{
	if (par[x] == x) return x;
	else return find(par[x]);
}

bool union_(int x, int y)
{
	x = find(x);
	y = find(y);

	if (x == y) return false;

	if (depth[x] > depth[y])
	{
		x ^= y;
		y ^= x;
		x ^= y;
	}

	par[x] = y;

	st.push(make_pair(make_pair(x, y), depth[x] == depth[y]));
	if (depth[x] == depth[y]) depth[y]++;

	return true;
}

void rollback(int cnt)
{
	int x, y;
	int add;

	while (cnt--)
	{
		x = st.top().first.first;
		y = st.top().first.second;
		add = st.top().second;
		st.pop();

		par[x] = x;
		depth[y] -= add;
	}
}

void recur(int idx, int s, int e)
{
	int mid;
	int i;
	int cur;
	int cnt = 0;

	if (s >= query.size()) return;
	if (s > e) return;

	for (i = 0; i < tree[idx].size(); i++)
	{
		cur = tree[idx][i];
		if (v[cur].second.first <= s && e <= v[cur].second.second)
		{
			cnt += union_(v[cur].first.first, v[cur].first.second);
		}
	}

	if (s == e)
	{
		if (find(query[s].first) == find(query[s].second)) cout << 1 << '\n';
		else cout << 0 << '\n';

		rollback(cnt);

		return;
	}

	mid = (s + e) / 2;

	recur(2 * idx, s, mid);
	recur(2 * idx + 1, mid + 1, e);

	rollback(cnt);
}

int main()
{
	int q;
	int a, b, c;
	int cnt = 0;
	int i;

	cin.tie(NULL);
	ios::sync_with_stdio(false);

	cin >> n >> q;
	while (q--)
	{
		cin >> a >> b >> c;

		if (b > c)
		{
			b ^= c;
			c ^= b;
			b ^= c;
		}

		if (a == 1)
		{
			m[b][c] = v.size();
			v.push_back(make_pair(make_pair(b, c), make_pair(cnt, len)));
		}
		else if (a == 2)
		{
			v[m[b][c]].second.second = cnt - 1;
		}
		else
		{
			query.push_back(make_pair(b, c));
			cnt++;
		}
	}

	for (i = 1; i <= n; i++) par[i] = i;

	for (i = 0; i < v.size(); i++)
	{
		if (v[i].second.first > v[i].second.second) continue;

		if (v[i].second.second == len) v[i].second.second = cnt - 1;

		update(1, 0, len - 1, v[i].second.first, v[i].second.second, i);
	}

	recur(1, 0, len - 1);
}
